...
export async function getAllHistoriques(filters: any = {}) {
    ...
    if (filters.descriptionAction) where.descriptionAction = { contains: filters.descriptionAction };
    if (filters.utilisateurId) where.utilisateurId = Number(filters.utilisateurId);
    if (filters.dateFrom || filters.dateTo) {
        where.dateModification = {};
        if (filters.dateFrom) where.dateModification.gte = new Date(filters.dateFrom);
