...
exports.getUserProfileService = exports.logout = exports.changePassword = exports.resetPassword = exports.generateResetToken = exports.verifyEmailService = exports.refreshAccessToken = void 0;
...
const generateResetToken = async (email) => {
    ...
    if (!user)
        return null;
    const token = jsonwebtoken_1.default.sign({ userId: user.id }, process.env.JWT_RESET_SECRET, {
        expiresIn: "1h",
    });
    return token;
};
...
const changePassword = async (userId, dto) => {
    ...
    const isMatch = await bcrypt_1.default.compare(dto.oldPassword, user.password);
    if (!isMatch) {
        return {
            statusCode: 400,
...
async function updateUserProfile(idUser, dto) {
    const existedUser = await (0, helpers_1.ensureExists)(() => prisma.users.findUnique({ where: { id: idUser } }), "User");
    const normalized = {
    ...
    const payload = (0, helpers_1.stripNullish)(normalized);
    ...
    if (payload.email) {
        ...
        if (other)
            throw new errors_1.ConflictError("Cette adresse e-mail est déjà utilisée.");
    }
    if (payload.phone) {
        const other = await prisma.users.findFirst({
        ...
        if (other)
            throw new errors_1.ConflictError("Ce numéro de téléphone est déjà utilisé.");
    }
    ...
    try {
        const updated = await prisma.$transaction(async (tx) => {
            ...
            await (0, historique_service_1.createHistoriqueService)(tx, idUser, emailChanged ? "EMAIL_CHANGED" : "PROFILE_UPDATED");
            return user;
        });
        if (emailChanged) {
            const verifToken = jsonwebtoken_1.default.sign({ sub: updated.id, email: updated.email }, accessSecret, { expiresIn: "1d" });
            try {
                await (0, email_1.sendVerificationEmail)(updated.email, verifToken);
            }
            catch (mailErr) {
